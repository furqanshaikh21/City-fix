...
import { UserPlus, ClipboardList, Loader, CheckCircle, MapPin, Upload, BarChart, MessageCircle, Users, CheckCheck } from 'lucide-react';
...
const features = [
  ...
  },
  {
    ...
    icon: <CheckCheck className="text-blue-500 w-6 h-6" />,
  },
];
...
const Home = () => {
  return (
    <div className="px-0">
       <HeroSection />
    <div className="mt-12 px-6 md:px-10 bg-gray-900 min-h-screen pb-20">
      
      {/* Header */}
      {/* <div className="text-center mb-20">
        <h1 className="text-5xl font-bold text-gray-100">Welcome to SmartCity</h1>
        <p className="mt-4 text-xl text-gray-100">
          Empowering citizens with technology to resolve issues faster.
        </p>
      </div> */}
      ...
      {/* Features Section */}
      <div className="text-center mb-20">
        <h2 className="text-4xl mt-36 font-bold text-gray-300 mb-3">Powerful Features for Community Action</h2>
        ...
        <div className="grid grid-cols-1 mt-16 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {features.map((feature, index) => (
            <div
              key={index}
              className="bg-gray-800 p-6 rounded-xl shadow-md hover:shadow-blue-400/30 transition duration-300 text-left"
            >
              <div className="mb-4 flex items-center justify-center w-12 h-12 rounded-full bg-blue-950/40">
                {feature.icon}
              </div>
      ...
      <div className="text-center mt-28">
        <h2 className="text-4xl font-bold text-gray-300 mb-3">How It Works</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-6xl mx-auto">
          {steps.map((step) => (
            <Link to={step.link} key={step.id}>
              <div
                className="relative group bg-gray-600 rounded-xl shadow-md p-8 text-left transition-transform duration-300 hover:scale-105 hover:shadow-blue-400/40 hover:shadow-lg"
              >
                {/* Step number badge */}
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-blue-600 text-white w-10 h-10 rounded-full flex items-center justify-center text-lg font-bold shadow-md transition-transform duration-300 group-hover:scale-110">
                  {step.id}
                </div>

                <div className="flex items-start space-x-4 mt-6">
                  <div className="p-3 bg-blue-100 rounded-full transition duration-300 group-hover:bg-blue-600">
                    {step.icon}
                  </div>
                  <div>
...
<div className="mt- bg-gray-900 text-white py-20 px-6 text-center rounded-xl shadow-lg mx-auto max-w-5xl">
  ...
  <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
    ...
    <Link
      to="/register"
      className="bg-white text-gray-600 hover:bg-gray-300 font-semibold py-3 px-6 rounded-md transition duration-300"
    >
      Join the Community
    </Link>
  </div>
